/*************************************************************************
 * inst-imul.cc -- This file is part of sasm.                            *
 * Copyright (C) 2021                                                    *
 *                                                                       *
...
size_t
AsmInstIMUL::width (const AsmContext &ctx)
{
  size_t pfsize = 0;
  ...
  return (op->width () != 1 && op->width () != ctx.bytes ? 2 : 1) +
    operand_width (AsmOpcodeExt::OE5, op, ctx) + pfsize;
}

bool
AsmInstIMUL::assemble (std::vector <unsigned char> &result,
		       const AsmContext &ctx)
{
  ...
  if (op->width () == 1)
    result.push_back (ASM_OPCODE_IMUL_AX_AL);
  else
    {
      if (op->width () != ctx.bytes)
	result.push_back (ASM_OPCODE_OPSIZE_OVR);
      result.push_back (ASM_OPCODE_IMUL_DX_AX);
    }
  return write_operand (AsmOpcodeExt::OE5, op, ctx, result);
}
