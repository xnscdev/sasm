/*************************************************************************
 * inst-neg.cc -- This file is part of sasm.                             *
 * Copyright (C) 2021                                                    *
 *                                                                       *
...
size_t
AsmInstNEG::width (const AsmContext &ctx)
{
  size_t pfsize = 0;
  ...
  return operand_width (AsmOpcodeExt::OE3, op, ctx) + pfsize +
    (op->width () != 1 && op->width () != ctx.bytes ? 2 : 1); 
}

bool
AsmInstNEG::assemble (std::vector <unsigned char> &result,
		      const AsmContext &ctx)
{
  ...
  if (op->width () == 1)
    result.push_back (ASM_OPCODE_NEG_RM8);
  else
    {
      if (op->width () != ctx.bytes)
	result.push_back (ASM_OPCODE_OPSIZE_OVR);
      result.push_back (ASM_OPCODE_NEG_RMX);
    }
  return write_operand (AsmOpcodeExt::OE3, op, ctx, result);
}
