/*************************************************************************
 * inst-callf.cc -- This file is part of sasm.                           *
 * Copyright (C) 2021                                                    *
 *                                                                       *
...
assemble_segaddr (std::vector <unsigned char> &result,
		  ...
		  uint32_t addr)
{
  result.push_back (ASM_OPCODE_CALLF_PX);
	    ...
	    const AsmContext &ctx, AsmMemoryLoc *loc)
{
  ...
  return write_operand (AsmOpcodeExt::OE3, loc, ctx, result);
}

size_t
AsmInstCALLF::width (const AsmContext &ctx)
{
  if (op == nullptr)
    return ctx.bytes + 3;
  else
...
bool
AsmInstCALLF::assemble (std::vector <unsigned char> &result,
