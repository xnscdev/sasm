/*************************************************************************
 * inst-dec.cc -- This file is part of sasm.                             *
 * Copyright (C) 2021                                                    *
 *                                                                       *
...
size_t
AsmInstDEC::width (const AsmContext &ctx)
{
  if (ISINSTANCE (AsmRegister *, op))
  ...
  else if (ISINSTANCE (AsmMemoryLoc *, op))
    {
      ...
      return operand_width (AsmOpcodeExt::OE1, op, ctx) +
...
bool
AsmInstDEC::assemble (std::vector <unsigned char> &result,
		      const AsmContext &ctx)
{
  if (ISINSTANCE (AsmRegister *, op))
      ...
      if (rop->width () == 1)
	{
	  ...
	  return write_operand (AsmOpcodeExt::OE1, op, ctx, result);
	}
      else
	{
	  if (rop->width () != ctx.bytes)
	    result.push_back (ASM_OPCODE_OPSIZE_OVR);
	  result.push_back (ASM_OPCODE_DEC_BASE + rop->id);
	  return true;
	}
    }
  else if (ISINSTANCE (AsmMemoryLoc *, op))
